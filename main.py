...
# USER AUTH
# ═══════════════════════════════════════════════════════════
ADMIN_EMAIL    = "pradeep@yourorg.com"
...
def render_login_page():
    ...
    .login-container{max-width:420px;margin:60px auto;background:white;border-radius:14px;
     padding:40px 36px;box-shadow:0 8px 40px rgba(0,0,0,.12);border-top:5px solid #B31B1B;}
    .login-title{font-family:'Rajdhani',sans-serif;font-size:26px;font-weight:700;color:#B31B1B;
...
def get_sample_dfs():
    c = pd.read_csv(StringIO(SAMPLE_CUSTOMERS_CSV))
    a = pd.read_csv(StringIO(SAMPLE_ACCOUNTS_CSV))
    t = pd.read_csv(StringIO(SAMPLE_TRANSACTIONS_CSV))
    return c, a, t
...
EXAMPLES = [
    ...
    {
        "tag": "✅ Top Customers", "complexity": "Simple",
...
{{"epic":{{"title":"","business_value":"","objective":"","estimated_sprints":3,"definition_of_done":[]}},"stories":[{{"id":"US-001","title":"","user_story":"As a [role], I want [feature], so that [benefit]","business_value":"","priority":"High","story_points":5,"points_rationale":"","sprint":"Sprint 1","type":"Feature","acceptance_criteria":["Given...When...Then..."],"definition_of_ready":[],"subtasks":[{{"title":"Analysis & Design","description":"","hours":6,"role":"Business Analyst / Tech Lead"}},{{"title":"Development","description":"","hours":10,"role":"Developer"}},{{"title":"Testing & QA","description":"","hours":5,"role":"QA Engineer"}},{{"title":"Deployment & Release","description":"","hours":2,"role":"DevOps / Developer"}},{{"title":"Documentation","description":"","hours":2,"role":"Developer / BA"}}]}}],"risks":[{{"title":"","description":"","likelihood":"Medium","impact":"High","mitigation":""}}],"dependencies":[],"sprint_plan":[{{"sprint":"Sprint 1","goal":"","stories":["US-001"],"total_points":5}}]}}

RULES:
- 4-7 stories
- Fibonacci points only (1/2/3/5/8/13)
- User story MUST follow: "As a [Business Persona], I want [capability], so that [measurable benefit]"
- Each story MUST have exactly 5 subtasks: Analysis & Design, Development, Testing & QA, Deployment & Release, Documentation
- Gherkin AC: each story needs 3 items: Given/When/Then happy path, edge case, error scenario
...
def _build_adf(story, epic_title):
    ac_items = [
        {"type": "listItem", "content": [{"type": "paragraph", "content": [{"type": "text", "text": str(ac)}]}]}
        for ac in story.get("acceptance_criteria", [])
    ]
    subtask_items = [
        {"type": "listItem", "content": [{"type": "paragraph", "content": [{"type": "text",
         "text": "{} (~{}h) — {}".format(st_item.get("title",""), st_item.get("hours","?"), st_item.get("role",""))}]}]}
        for st_item in story.get("subtasks", [])
    ]
    content_blocks = [
        {"type": "paragraph", "content": [{"type": "text", "text": story.get("user_story",""), "marks": [{"type": "em"}]}]},
        {"type": "paragraph", "content": [{"type": "text", "marks": [{"type": "strong"}],
         "text": "Sprint: {} | Points: {} | Priority: {} | Epic: {} | Type: {}".format(
             story.get("sprint",""), story.get("story_points",""),
             story.get("priority",""), epic_title, story.get("type",""))}]},
    ]
    bv = story.get("business_value","")
    if bv:
        content_blocks.append({"type": "paragraph", "content": [{"type": "text", "text": "💡 Business Value: " + bv}]})
    if ac_items:
        content_blocks.append({"type": "heading", "attrs": {"level": 3}, "content": [{"type": "text", "text": "✅ Acceptance Criteria (Gherkin)"}]})
        content_blocks.append({"type": "bulletList", "content": ac_items})
    if subtask_items:
        content_blocks.append({"type": "heading", "attrs": {"level": 3}, "content": [{"type": "text", "text": "🔧 SDLC Subtasks"}]})
...
def push_story_to_jira(story, epic_title, jira_url, jira_email, jira_token, jira_project_key):
    ...
    summary   = "{} - {}".format(story.get("id","US"), story.get("title","")).encode("ascii","ignore").decode("ascii")
    labels    = [story.get("type","Feature").replace(" ","-"), story.get("sprint","Sprint-1").replace(" ","-")]
    parent_payload = {
        "fields": {
    ...
    subtask_keys = []
    for i, st_item in enumerate(story.get("subtasks", []), 1):
        st_summary = "{}.{} - {} (~{}h)".format(
            story.get("id","US"), i, st_item.get("title",""), st_item.get("hours",0)
        ...
        st_payload = {
            "fields": {
                ...
                "description": {"type":"doc","version":1,"content":[{"type":"paragraph","content":[{"type":"text",
                    "text": "Subtask of: {} | ~{}h | Role: {}".format(parent_key, st_item.get("hours","?"), st_item.get("role",""))}]}]},
...
st.markdown(f"""<div class="session-bar">
    ...
    <div>📡 Router: <span class="sb-val" style="color:#69F0AE;">REST API (no litellm)</span></div>
...
with tab1:
    st.markdown("""<div style="background:#F0F7FF;border:1px solid #BBDEFB;border-left:4px solid #1E90FF;border-radius:8px;padding:12px 16px;margin-bottom:16px;">
        ...
        <div style="font-size:12px;color:#333;line-height:1.9;">
            ...
            <b>Step 4:</b> AI generates Python code. Code executes locally on your original data.<br>
    ...
    if etl_raw:
        ...
        if pii_in_prompt:
            st.markdown(
                f'<div class="pii-warning"><div class="pw-title">⚠️ PII Detected in Prompt: {", ".join(pii_in_prompt)}</div>'
                f'<div style="font-size:12px;color:#555;">Auto-redacted before sending to AI.</div></div>',
                unsafe_allow_html=True
            )

    if st.session_state.get("using_sample"):
        st.info("💡 Using embedded sample data. Upload your own CSVs below to override.")
    ...
    uploaded = st.file_uploader(
        "Upload CSV File(s) — max 50MB each (optional if using sample data)",
    ...
    with st.expander("🤖 AI Provider Status", expanded=False):
        ...
        is_g_next = "Gemini" in np_val
        c_next.markdown(
            f"<div style='background:{'#E8F5E9' if is_g_next else '#FFF3E0'};border:1px solid {'#A5D6A7' if is_g_next else '#FFB300'};border-radius:6px;padding:8px 12px;font-size:12px;'>"
            f"⏭️ <b>Next call:</b><br><span style='font-size:14px;font-weight:700;color:{'#2E7D32' if is_g_next else '#E65100'};'>{np_val}</span></div>",
            unsafe_allow_html=True
        )
        c_last.markdown(
            f"<div style='background:#E3F2FD;border:1px solid #90CAF9;border-radius:6px;padding:8px 12px;font-size:12px;'>"
            f"✅ <b>Last call:</b><br><span style='font-size:14px;font-weight:700;color:#1565C0;'>{lu_val}</span></div>",
            unsafe_allow_html=True
        )
        ...
        if btn2.button("⚡ Reset All Cooldowns", key="reset_cd_tab1"):
            n = reset_cooldowns()
            st.success(f"✅ Cleared {n} cooldown(s)")
            st.rerun()
    ...
    with hc:
        st.markdown(
            ...
            f"Next: <b style='color:#2E7D32;'>{get_next_provider()}</b> &nbsp;·&nbsp; "
            f"Zero data sent to AI &nbsp;·&nbsp; 📡 Direct REST API</div>",
            unsafe_allow_html=True
        )
    ...
    if run:
        ...
        with st.spinner(f"⚙️ {get_next_provider()} generating pipeline (schema only — no data values sent)..."):
            for attempt in range(1, 3):
                ...
                if ai_code == RATE_LIMIT_SENTINEL:
                    show_ai_error("ETL code generation")
                    st.stop()
                try:
        ...
        if "Gemini" in actual_prov:
            ...
            st.warning(f"⚠️ **Answered by: {actual_prov}** ({actual_model})")
        ...
        ci.markdown(
            f"<span style='font-size:13px;color:#666;'>Total: <b>{len(result_df):,}</b> rows"
            f" &nbsp;·&nbsp; Provider: <b style='color:#1565C0;'>{actual_prov}</b></span>",
            unsafe_allow_html=True
        )
        total = len(result_df)
...
with tab2:
    ...
    st.markdown(
        ...
        f"📊 <b>Team velocity:</b> ~{vel} pts/sprint &nbsp;·&nbsp; 🏷 <b>Subtasks:</b> Analysis → Dev → Testing → Deployment → Docs</div>",
        unsafe_allow_html=True
    )
    ...
    jira_raw = st.text_area(
        ...
        placeholder="Example: Build a customer portal allowing bank customers to view statements, download PDFs, raise disputes, and set up standing orders..."
    )
    ...
    if jira_raw:
        ...
        if pj:
            st.markdown(
                f'<div class="pii-warning"><div class="pw-title">⚠️ PII in Requirement: {", ".join(pj)}</div>'
                f'<div style="font-size:12px;color:#555;">Auto-redacted before sending to AI.</div></div>',
                unsafe_allow_html=True
            )
    ...
    if st.button("🚀 Generate Jira Breakdown", key="run_jira", use_container_width=False):
        if not jira_raw.strip():
            ...
            st.stop()
        with st.spinner(f"🧠 Generating via {get_next_provider()} (temperature=0.1)..."):
        ...
        if raw_out == RATE_LIMIT_SENTINEL:
            show_ai_error("Jira breakdown generation")
            st.stop()
        ...
        if not jdata:
            with st.spinner("⚠️ AI returned non-JSON — auto-retrying..."):
                retry_messages = [
                    {"role": "system", "content": "You are a JSON-only API. Return ONLY a valid JSON object. Start with { and end with }. No markdown, no explanation."},
                    {"role": "user", "content": f"Extract and return ONLY the JSON object from this text:\n\n{raw_out[:3000]}"},
                ]
                ...
                if raw_out2 and raw_out2 != RATE_LIMIT_SENTINEL:
                    ...
                    if jdata:
                        st.success("✅ Auto-retry successful.")
    ...
    if st.session_state.jira_result:
        ...
        dod_html = "".join(
            f'<div style="font-size:12px;color:rgba(255,255,255,.8);padding:2px 0;">✓ {d}</div>'
            for d in epic.get("definition_of_done", [])
        )
            ...
            for si, sp_item in enumerate(sprint_plan):
                with sp_cols[si % 3]:
                    st.markdown(f'''<div class="sprint-plan-card">
                        ...
                        <div style="font-size:11px;color:#888;">{", ".join(sp_item.get("stories",[]))}</div>
                    </div>''', unsafe_allow_html=True)

        st.markdown('<div class="section-title">📖 User Stories</div>', unsafe_allow_html=True)
        edit_mode = st.toggle("✏️ Enable PO Edit Mode", key="edit_toggle")
        ...
        if edit_mode:
            st.markdown('<div class="jira-edit-panel"><b style="color:#B31B1B;">✏️ PO EDIT MODE ACTIVE</b> — All fields are editable.</div>', unsafe_allow_html=True)
            ...
            for i, s in enumerate(stories):
                with st.expander(f"✏️ {s.get('id','US')} — {s.get('title','')}", expanded=False):
                    ...
                    with col1:
                        ...
                        new_us     = st.text_area("User Story", value=s.get("user_story",""), key=f"s_us_{i}", height=90)
                    ...
                    new_ac_raw = st.text_area("AC (Gherkin, one per line)", value=ac_text, key=f"s_ac_{i}", height=120)
                    new_ac     = [l.strip() for l in new_ac_raw.split("\n") if l.strip()]
                    st.markdown("**SDLC Subtask Hours**")
                    edited_subtasks = []
                    default_subtasks = [
                        {"title": "Analysis & Design",   "hours": 6,  "role": "Business Analyst / Tech Lead", "description": ""},
                        {"title": "Development",          "hours": 10, "role": "Developer",                   "description": ""},
                        {"title": "Testing & QA",         "hours": 5,  "role": "QA Engineer",                 "description": ""},
                        {"title": "Deployment & Release", "hours": 2,  "role": "DevOps / Developer",          "description": ""},
                        {"title": "Documentation",        "hours": 2,  "role": "Developer / BA",              "description": ""},
                    ]
                    ...
                    for si2, (stc, stl) in enumerate(zip(current_subtasks[:5], st_labels)):
                        new_hrs = st_cols[si2].number_input(
                            ...
                            value=int(stc.get("hours",4)), key=f"s_{i}_st_{si2}"
                        )
                        edited_subtasks.append({**stc, "hours": new_hrs})
                    edited_stories.append({
                        "id": s.get("id",f"US-{i+1:03d}"), "title": new_title,
                        "user_story": new_us, "business_value": new_bv,
                        "priority": new_pri, "story_points": new_pts,
                        "points_rationale": s.get("points_rationale",""),
                        "sprint": new_sprint, "type": s.get("type","Feature"),
            ...
            if st.button("💾 Save All Edits", key="save_edits"):
                updated = dict(jd)
                updated["epic"] = {
                    "title": new_epic_title, "business_value": new_epic_bv,
                    "objective": new_epic_obj, "estimated_sprints": new_sprints,
            ...
            for s in stories:
                ...
                with st.expander(f"  {s.get('id','US')} · {s.get('title','')}  [{pri}] [{pts}pts] [{s.get('sprint','')}]", expanded=False):
                    st.markdown(f'<div class="user-story-block">💬 {s.get("user_story","")}</div>', unsafe_allow_html=True)
                    bv = s.get("business_value","")
                    if bv:
                        st.markdown(f'<div class="biz-value-block">💡 <b>Business Value:</b> {bv}</div>', unsafe_allow_html=True)
                    st.markdown(
                    ...
                    if pr_text:
                        st.markdown(f'<div class="points-rationale-block">📊 {pr_text}</div>', unsafe_allow_html=True)
                    ...
                    st.markdown(f'<div class="ac-section"><div class="ac-title">✅ Acceptance Criteria — Gherkin</div>{acs_html}</div>', unsafe_allow_html=True)
                    ...
                    if dor:
                        ...
                        st.markdown(f'<div style="margin-top:10px;"><div style="font-size:11px;font-weight:700;color:#1565C0;text-transform:uppercase;margin-bottom:5px;">📋 Definition of Ready</div>{dor_html}</div>', unsafe_allow_html=True)
                        ...
                        for st_item in subtasks:
                            ...
                            subtask_rows += f'''<div class="subtask-row">
                                ...
                                <div class="subtask-content">
                                    ...
                                    <div class="subtask-meta">~{st_item.get("hours","?")}h &nbsp;·&nbsp; {st_item.get("role","")}</div>
                                    <div class="subtask-desc">{st_item.get("description","")}</div>
                                </div>
                            </div>'''
                        st.markdown(f'<div class="sdlc-section"><div class="sdlc-title">🔧 SDLC Subtasks — {total_hours}h estimated</div>{subtask_rows}</div>', unsafe_allow_html=True)
        ...
        with export_tab1:
            ...
            if tc1.button("🔌 Test Connection", key="test_jira"):
                if not all([jira_url, jira_email, jira_token, jira_proj]):
                    ...
                    try:
                        r_test = requests.get(
                            "{}/rest/api/3/project/{}".format(jira_url.rstrip("/"), jira_proj),
                            auth=(jira_email, jira_token), timeout=10
                        )
                        if r_test.status_code == 200:
                            pdata = r_test.json()
                            st.success(f"✅ Connected! Project: **{pdata.get('name', jira_proj)}**")
                        elif r_test.status_code == 401:
                            st.error("❌ 401 Unauthorized — check email/token.")
                        elif r_test.status_code == 404:
                            st.error(f"❌ 404 Project '{jira_proj}' not found.")
                        else:
            ...
            if st.button("🚀 Push All Stories + SDLC Subtasks to Jira", key="push_jira", use_container_width=True):
                if not all([jira_url, jira_email, jira_token, jira_proj]):
                    st.error("❌ Fill all Jira connection fields.")
                else:
                    ...
                    st.info(f"📤 Pushing {len(current_stories)} stories + {total_subtasks} subtasks...")
                    success_count, fail_count, total_subtask_created = 0, 0, 0
                    ...
                    for idx, story in enumerate(current_stories):
                        ...
                        total_subtask_created += len(subtask_keys)
                        results_log.append({
                            "Story ID": story.get("id","?"),
                            "Title": story.get("title","")[:40],
                            "Jira Key": parent_key if ok else "-",
                            "SDLC Subtasks": len(subtask_keys),
                            "Points": story.get("story_points","?"),
                            "Sprint": story.get("sprint","?"),
                            "Status": "✅ Created" if ok else f"❌ {parent_key[:60]}",
                        })
                        if ok:
                            success_count += 1
                        else:
                    ...
                    if success_count > 0:
                        st.success(f"✅ {success_count} stories + {total_subtask_created} subtasks pushed!")
                    if fail_count > 0:
                        st.warning(f"⚠️ {fail_count} stories failed.")
                    st.dataframe(pd.DataFrame(results_log), use_container_width=True, hide_index=True)
                    audit_log("JIRA_EXPORT", SESSION_ID,
                              f"User={CURRENT_USER},Project={jira_proj},Stories={success_count},Subtasks={total_subtask_created}",
                              "MEDIUM")
        ...
        with export_tab2:
            ...
            txt_lines = [f"EPIC: {current_epic.get('title','')}", f"Business Value: {current_epic.get('business_value','')}", ""]
            for s in current_stories:
                txt_lines += [
                    f"{'='*60}", f"{s.get('id','')} — {s.get('title','')}",
                    f"User Story: {s.get('user_story','')}", f"Priority: {s.get('priority','')} | Points: {s.get('story_points','')}",
                    "", "ACCEPTANCE CRITERIA:",
                ]
            ...
            with pd.ExcelWriter(xb, engine="xlsxwriter") as w:
                pd.DataFrame([{
                    ...
                    "User Story": s.get("user_story",""), "Business Value": s.get("business_value",""),
                    "Priority": s.get("priority",""), "Story Points": s.get("story_points",""),
                    "Sprint": s.get("sprint",""), "Type": s.get("type",""),
                    "AC": "\n".join(s.get("acceptance_criteria",[])),
                ...
                subtask_rows = [
                    {"Story ID": s.get("id",""), "Story": s.get("title","")[:40],
                     "Subtask": st_item.get("title",""), "Description": st_item.get("description",""),
                     "Hours": st_item.get("hours",""), "Role": st_item.get("role","")}
                    for s in current_stories for st_item in s.get("subtasks",[])
                ]
                if subtask_rows:
...
with tab3:
    ...
    st.markdown("""<div style="display:grid;grid-template-columns:repeat(4,1fr);gap:14px;">
...
with tab4:
    ...
    is_g_next = "Gemini" in np_val
    col_next.markdown(
        f"<div style='background:{'#E8F5E9' if is_g_next else '#FFF3E0'};border:1px solid {'#A5D6A7' if is_g_next else '#FFB300'};border-radius:8px;padding:12px 16px;'>"
        f"<div style='font-size:11px;color:#666;margin-bottom:4px;'>⏭️ NEXT CALL</div>"
        f"<div style='font-size:16px;font-weight:700;color:{'#2E7D32' if is_g_next else '#E65100'};'>{np_val}</div></div>",
        unsafe_allow_html=True
    )
    col_last.markdown(
    ...
    if test_in:
        ...
        if found:
            st.markdown(
                f'<div class="pii-warning"><div class="pw-title">⚠️ PII Detected: {", ".join(found)}</div>'
                f'<div style="font-size:12px;color:#555;margin-top:6px;">'
                f'<b>Original:</b> {test_in}<br><b>Sanitized:</b> {san}</div></div>',
                unsafe_allow_html=True
            )
        else:
            st.success("✅ No PII detected. Text passes through unchanged.")

    st.markdown('<div class="section-title">🔍 Last AI Error (Debug)</div>', unsafe_allow_html=True)
    last_err_display = get_last_error()
    if last_err_display:
        st.error(f"**Last Error:**\n```\n{last_err_display}\n```")
    else:
        st.success("✅ No errors on record.")
